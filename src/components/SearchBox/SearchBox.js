...
class SearchBox extends Component {
    ...
    searchLineChangeHandler = (e) => {
        ...
        if (input) {
            input = this.transliterate(input)
              ...
              input = this.transliterate(input, true)
        };
      
        kirillicaChange = (input) => {
            transliterate(input)
            return input
        }

        this.setState({ searchLine: input });
    ...
    searchBoxSubmitHandler = (e) => {
        ...
        fetch(`http://www.omdbapi.com/?s=${this.state.searchLine}&apikey=341e2618 `)
           .then((res) => res.json())
           .then((data) => {
